...
def buildTarget(mxy, mz, B1, dt, bwTB, sharpnessTB, phSlopReduction):
    ...
    if max_tAx == 0:
        ...
        fAx = np.zeros_like(tAx)
    else:
        fAx = np.linspace(-(1. / max_tAx) * len(tAx), +(1. / max_tAx) * len(tAx), len(tAx)) * 1e-6 * 2 * np.pi
    ...
    peaks_input_xy = np.abs(1. / (np.abs(mxy) - 0.5 + 1e-9)) # Added small epsilon to avoid division by zero
    ...
    if len(locsXY) % 2 != 0:
         print("Warning: Odd number of peaks found for Mxy. Reshaping might fail or be incorrect.")
    ...
    peaks_input_z = np.abs(1. / (np.abs(mz) - 0.5 + 1e-9)) # Added small epsilon
    ...
    if len(fAx) > 1:
      df = fAx[1] - fAx[0]
      fTb = np.arange(-bwTB / 2, bwTB / 2 + df, df) # Ensure endpoint is included if step divides evenly
    else:
      # Handle case with single point fAx if necessary
      df = 0
      fTb = np.array([-bwTB / 2, bwTB / 2]) # Or define as appropriate
    ...
    if bwTB == 0:
        ...
        transProf = np.ones_like(fTb) # Example: all ones
    else:
        transFct = lambda x, n: (0.5 * np.sin(x / (bwTB / 2) * (np.pi / 2)) + 0.5)**n
        transProf = transFct(fTb, sharpnessTB)

    lenTransProf = len(transProf)

    # Stitch new profile together [cite: 10, 11, 12, 13, 14, 15, 16]
    targetProfMz = np.zeros_like(fAx)
    targetProfMxy = np.zeros_like(fAx)
    ...
    for idx in range(idxPosXY.shape[1]): # Iterate through columns (pairs of indices)
        ...
        min_idx_z, max_idx_z = np.min(idx_z_pair), np.max(idx_z_pair)
        min_idx_xy, max_idx_xy = np.min(idx_xy_pair), np.max(idx_xy_pair)
        ...
        profToAddRight = np.flip(transProf) # Matlab fliplr equivalent [cite: 13]
        ...
        current_range_indices = np.arange(min_idx_z, max_idx_z + 1)
        if len(current_range_indices) > 1: # polyfit needs at least 2 points
            xVec = np.arange(len(current_range_indices)) - len(current_range_indices) / 2.0
            # Use mxy within the Z-defined band for phase fitting
            phase_data = np.unwrap(np.angle(mxy[current_range_indices]))
            ...
            current_targetPhase = phEvol_coeffs[1] + phEvol_coeffs[0] * phSlopReduction * xVec
            targetProfPhase_list.append(current_targetPhase)
        else:
             # Handle cases with insufficient points if necessary
             targetProfPhase_list.append(np.array([np.angle(mxy[min_idx_z])])) # Append single phase value
    ...
    if targetProfPhase_list:
         ...
         targetProfPhase = np.zeros_like(fAx) # Default if no bands processed
    ...
    targPhMxy = np.zeros_like(fAx) # Initialize
    ...
    for idx in range(idxPosXY.shape[1]):
        min_idx_xy, max_idx_xy = np.min(idxPosXY[:, idx]), np.max(idxPosXY[:, idx])
        if phase_idx_count < len(targetProfPhase_list):
             ...
             if len(phase_slice) == target_len:
                 ...
                 print(f"Warning: Phase profile length ({len(phase_slice)}) mismatch for Mxy band ({target_len}). Using partial assignment.")
                 assign_len = min(len(phase_slice), target_len)
                 targPhMxy[min_idx_xy : min_idx_xy + assign_len] = phase_slice[:assign_len]

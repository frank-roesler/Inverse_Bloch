...
def loss_fn(z_profile, xy_profile, tgt_z, tgt_xy, pulse, gradient):
    ...
    # L2 norm
    #L2_loss_mxy = torch.mean( ( (xy_profile_abs - tgt_xy) ** 2 ) ** 1/5 ) 
    #L2_loss_mz = torch.mean( ( (z_profile - tgt_z) ** 2) ** 1/5 )
    # L1 norm
    L2_loss_mxy = torch.mean( ( abs(xy_profile_abs - tgt_xy) ) ** 1/6 ) 
    L2_loss_mz = torch.mean( ( abs(z_profile - tgt_z) ) ** 1/6 )

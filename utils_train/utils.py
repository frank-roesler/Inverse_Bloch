...
class InfoScreen:
    ...
    def init_plots(self):
        self.fig, self.ax = plt.subplots(1, 3, figsize=(12, 3.5), constrained_layout=True)
        ...
        self.q = self.ax[1].plot([0], [1e-4], linewidth=1, label="Pulse shape")[0]
    ...
    def plot_info(self, epoch, losses, fAx, t_B1, target_z, target_xy, mz, mxy, pulse, gradient):
        ...
        if epoch % self.output_every == 0:
            ...
            self.ax[0].set_ylim(
                (
                    1.1 * np.min([np.min(mz_plot), np.min(mxy_abs), np.min(error)]),
                    1.1 * np.max([np.max(mz_plot), np.max(mxy_abs), np.max(error)]),
                )
            )
            self.ax[1].set_ylim((1.1 * np.min(np.abs(pulse_real)).item(), 1.1 * np.max(np.abs(pulse_real)).item()))
            self.ax[2].set_ylim((0.9 * np.min(losses), 1.1 * np.max(losses)))
    ...
    def print_info(self, epoch, L2_loss):
        self.t1 = time() - self.t0
        self.t0 = time()
        print("Epoch: ", epoch)
        print(f"Loss: {L2_loss.item():.3f}")
...
def loss_fn(z_profile, xy_profile, tgt_z, tgt_xy):

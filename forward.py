...
target_z, target_xy, slice_centers, half_width = get_smooth_targets(
    theta=flip_angle, smoothness=3.0, function=torch.sigmoid, n_targets=n_slices, pos=fixed_inputs["pos"]
)
...
for Nt in [256]:
    ...
    fixed_inputs = get_fixed_inputs(tfactor=tfactor, n_b0_values=1, Nz=256, Nt=Nt)
    ...
    # n_slices = 2
    # target_z, target_xy, slice_centers, half_width = get_smooth_targets(
    #     theta=flip_angle, smoothness=2.0, function=torch.sigmoid, n_targets=n_slices, pos=fixed_inputs["pos"]
    # )
